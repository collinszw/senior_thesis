...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  */
  ...
  for ( const auto& fp : ev.get_final_particles() ) {
    ...
    if(fp->pdg_code() == 11){
      fParticle = new G4PrimaryParticle(fp->pdg_code(),fp->px(), fp->py(), fp->pz(), fp->total_energy() );
      //fParticle->SetPDGcode(fp->pdg_code());
      //fParticle->Set4Momentum(fp->px(), fp->py(), fp->pz(), fp->total_energy() );
      fParticle->SetCharge( fp->charge() );
      
      vertex->SetPrimary(fParticle);
